...
###### On the Fourth Session (Client)
 ...
 - When running this command, you'll likely see two error messages: java.io.FileNotFoundException and org.json.JSONException.  These can safely be ignored, as the "missing" file (settings.json) isn't actually needed by the client.  Simply wait until the Average Delay metric is displayed.  Or (since the client command doesn't contain any relative paths) you can run the command from within any one of the etherpad-lite-X folders and the errors won't arise.
